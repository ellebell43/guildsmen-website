...
export default function PageFooter(props: { active: "character" | "skills" | "gear" | "details" | "notes" | "settings", setActive: Function }) {
  const buttonClass = "border-stone-400 dark:border-stone-200 w-full py-2 hover:bg-stone-300 dark:hover:bg-stone-500 hover:cursor-pointer lg:h-full lg:flex lg:gap-2 lg:items-center lg:pl-2"
  const iconClass = "text-[18px] max-w-[20px]"
  return (
    <div className="fixed bottom-0 right-0 lg:right-auto lg:top-16 left-0 lg:w-[100px] border-t lg:border-r border-stone-800 dark:border-stone-200 bg-stone-200 dark:bg-stone-600 flex lg:flex-col justify-evenly transition-all">
      <button className={`${buttonClass}  ${props.active == "character" ? "bg-stone-300 dark:bg-stone-500" : ""}`} onClick={e => { props.setActive("character"); window.scrollTo(0, 0) }}>
        ...
        <p className="m-0 text-xs lg:text-base text-center">Stats</p>
      </button>
